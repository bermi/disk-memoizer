...
// Used to convert md5 paths into subfolder chunks
...
function diskMemoizer(unmemoizedFn, {
  ...
  return function diskMemoized(identity, callback) {
    ...
    const marshaller = marshallers[type] || marshallers.none;
...
function getCachePath(key) {
  return path.normalize(
    `${config.CACHE_DIR}/${createHash("md5").
      update(key).
      digest("hex").
...
function grabAndCache({
  ...
  unmemoizedFn(key, (err, unmarshalledData) => {
    ...
    marshaller = marshallers[type] || marshallers.none;
